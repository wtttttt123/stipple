...
class Stippler():
	...
	def SASD(self,G0,G1,k,D,img,deep):
		...
		self.aheap,self.M=self.build_heap_depth(img,edge_list)
		stipplelist={}
		while (self.aheap):
			pixel=heappop(self.aheap)
			P,loc,I=pixel[0],pixel[1],pixel[2]
			if P!=-self.calculate_priority_depth(img[loc[0],loc[1]],loc,edge_list):
					heappush(self.aheap,(-self.calculate_priority_depth(img[loc[0],loc[1]],loc,edge_list),loc,img[loc[0],loc[1],0]))
			else:
				if not self.M[loc]:
	...
	def enlarge(self,stippleimg):
		...
		mpimg.imsave(output_name, new_resolution/255)
...
if __name__== "__main__":
	...
	stippler1=Stippler("OO");
	result=stippler1.SASD(5,5,0,7,man255,resdeep)
